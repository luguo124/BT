...
if [ -f "/usr/bin/apt-get" ];then
	...
	if [ "$isDebian" != "" ];then
		wget -O install.sh https://download.btpanel.net/install/install-ubuntu.sh && bash install.sh
		exit;
	else
		wget -O install.sh https://download.btpanel.net/install/install-ubuntu.sh && sudo bash install.sh
		exit;
	fi
fi
...
wget -O panel.zip $btpanel_Url/install/src/panel.zip -T 10
